...
<table class="table table-bordered text-center table-sm">
	...
	<%for(Training t : train){ %>
		...
		<form method="post" action="/TaxiDriverSystem/Training">
			<td><%=t.getTid()%><input type="hidden" name="tid" value="<%=t.getTid()%>"></td>
			...
			<td><input  class="form-control" type="date" name="expirydate" value="<%=df.format(t.getDateOfExpiry())%>" required></td>
			<td><button class="btn btn-primary" name="tfunc" value="update">update</button></td>
			<td><button  class="btn btn-primary" name="tfunc" value="delete">delete</button></td>
		</form>
		</tr>
	<%} %>
	<form method="post" action="/TaxiDriverSystem/Training">
			...
			<td><input  class="form-control" type="date" name="expirydate" required></td>
			<td colspan="2"><button class="btn btn-primary" name="tfunc" value="add">add</button></td>
		</form>
</table>
