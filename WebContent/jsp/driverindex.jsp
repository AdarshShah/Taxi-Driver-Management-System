...
<body>
	<div class="jumbotron">
		...
		<div class="row">
			<form class="form-inline col-6 mx-auto" method="post"
				action="/TaxiDriverSystem/DriverCRUD">
				<input class="form-control mr-2" type="text" name="license"
					placeholder="License No" required="required">
				<button class="btn btn-primary" name="function" value="search">Search</button>
			</form>
			<form class="form-inline col-6" method="post"
				action="/TaxiDriverSystem/DriverCRUD">
				<button class="btn btn-primary" name="function" value="expire">License
					Expiring Candidates</button>
			</form>
		</div>
			...
			%>
				<tr>
					<form method="post" action="/TaxiDriverSystem/DriverCRUD">
						<td><%=d.getDriverId() %><input type="hidden" name="driverId"
							value="<%=d.getDriverId() %>"></td>
						<td><input type="text" class="form-control" name="name"
							value="<%=d.getDriverName() %>" required="required" /></td>
						<td><input type="text" class="form-control" name="license"
							value="<%=d.getLicense() %>" required="required" /></td>
						<td><input type="text" class="form-control" name="contact"
							value="<%=d.getContact() %>" required="required" /></td>
	...
	%>
				<tr>
					<form method="post" action="/TaxiDriverSystem/DriverCRUD">
						...
						<td><input type="hidden" name="tsid"
							value="<%=ts.getTsId() %>">
							<button class="btn btn-primary" name="tsfunc" value="delete">Delete</button></td>
						<td><button class="btn btn-primary" name="tsfunc"
								value="manage">Manage</button></td>
					</form>
				</tr>
				<%} %>
				<thead class="thead-dark">
				...
				<tr>
					<form method="post" action="/TaxiDriverSystem/DriverCRUD">
						...
						<td><input type="date" class="form-control"
							name="sessiondate" required></td>
						<td colspan="2"><button class="btn btn-primary" name="tsfunc"
