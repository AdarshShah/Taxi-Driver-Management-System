			...
			<table class="table table-bordered text-center table-sm">
				...
				<tr>
					<form method="post" action="/TaxiDriverSystem/Qualification">
						<td><%=q.getQid()%><input type="hidden" name="qid"
							value="<%=q.getQid()%>"></td>
						<td><select class="form-control" name="type" required>
								<option value="Central_London"
									<%=q.getQualification()==enumQualification.Central_London?"selected":"" %>>Central
									London</option>
								<option value="North_London"
									<%=q.getQualification()==enumQualification.North_London?"selected":"" %>>North
									London</option>
								<option value="East_London"
									<%=q.getQualification()==enumQualification.East_London?"selected":"" %>>East
									London</option>
								<option value="South_London"
									<%=q.getQualification()==enumQualification.South_London?"selected":"" %>>South
									London</option>
								<option value="West_London"
									<%=q.getQualification()==enumQualification.West_London?"selected":"" %>>West
									London</option>
						</select></td>
						<td><input class="form-control" type="date" name="expirydate"
							value="<%=df.format(q.getDateOfExpiry())%>" required></td>
						<td><button class="btn btn-primary" name="qfunc"
				...
				<form method="post" action="/TaxiDriverSystem/Qualification">
					...
					<td><input type="date" class="form-control" name="expirydate"
						required></td>
					<td colspan="2"><button class="btn btn-primary" name="qfunc"
